...
function App() {
  ...
  const getBasicInfo = async () => {
    const unisat = (window as any).opnet;
  ...
  useEffect(() => {
    ...
    async function checkUnisat() {
      let unisat = (window as any).opnet;

      for (let i = 1; i < 10 && !unisat; i += 1) {
          await new Promise((resolve) => setTimeout(resolve, 100*i));
          unisat = (window as any).opnet;
      }
  ...
  return (
    <Card size="small" title="Sign Psbt" style={{ width: 300, margin: 10 }}>
      ...
      <Button
        ...
        onClick={async () => {
          try {
            const psbtResult = await (window as any).opnet.signPsbt(psbtHex);
  ...
  return (
    <Card size="small" title="Sign Message" style={{ width: 300, margin: 10 }}>
      ...
      <Button
        style={{ marginTop: 10 }}
        onClick={async () => {
          const signature = await (window as any).opnet.signMessage(message);
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        onClick={async () => {
          try {
            const txid = await (window as any).opnet.pushTx(rawtx);
            setTxid(txid);
          } catch (e) {
  ...
  return (
    <Card size="small" title="Push Psbt Hex" style={{ width: 300, margin: 10 }}>
      ...
      <Button
        ...
        onClick={async () => {
          try {
            const txid = await (window as any).opnet.pushPsbt(psbtHex);
            setTxid(txid);
          } catch (e) {
            setTxid((e as any).message);
          }
  ...
  return (
    <Card size="small" title="Send Bitcoin" style={{ width: 300, margin: 10 }}>
      ...
      <Button
        ...
        onClick={async () => {
          try {
            const txid = await (window as any).opnet.sendBitcoin(
              toAddress,
              satoshis
            );
            setTxid(txid);
